...
class AppSignatureValidator: NSObject {
    ...
    static func warn(reason: String) {
        ...
        let message = if team == nil {
            "A required file appears to be missing or corrupted and Tidey’s code signature could not be verified.\n\nYou should download a fresh copy of the app and reinstall it."
        } else if team == "H7V7XYVQ7D" {
            "A required file appears to be missing or corrupted and Tidey’s code signature did not match that of the official distribution.\n\nYou should download a fresh copy of the app and reinstall it."
        } else {
            "A required file appears to be missing or corrupted, yet against all odds the code signature for Tidey is valid. Please file a bug at https://tidey.app/tbd"
        }
        let alert = NSAlert()
