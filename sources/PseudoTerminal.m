...
static const NSTimeInterval TideyRuntimeProbeTimeout = 3;
static const NSTimeInterval TideyRuntimeMutationTimeout = 10;

static NSDictionary<NSString *, NSString *> *
TideyRuntimeTaskEnvironment(void) {
    NSString *socketPath = [TideySocketServer socketPath] ?: @"";
    NSString *binDirectory =
        [[[NSBundle mainBundle] resourcePath]
            stringByAppendingPathComponent:@"bin"] ?: @"";
    TideyRuntimeTaskEnvironmentBuilder *builder =
        [[[TideyRuntimeTaskEnvironmentBuilder alloc] init] autorelease];
    return [builder
...
- (void)probeTargetForDescriptor:(TideyRuntimeResumeDescriptor *)descriptor
    ...
    dispatch_async(_queue, ^{
        ...
        TideyRuntimeTaskExecutionResult *result = nil;
        for (NSInteger attempt = 1; attempt <= 2; attempt++) {
            result = TideyRuntimeRunTmuxTask(
                descriptor,
                arguments,
                TideyRuntimeProbeTimeout
            );
            BOOL isMissing = result.launched &&
                !result.timedOut &&
                result.terminationReason == NSTaskTerminationReasonExit &&
                result.terminationStatus == 1;
            if (result.succeeded || isMissing || attempt == 2) {
                break;
            }
            DLog(@"[TideyRuntimeRehydration] phase=has-session "
...
- (void)createTopologyForDescriptor:(TideyRuntimeResumeDescriptor *)descriptor
    ...
    dispatch_async(_queue, ^{
        BOOL succeeded = [self createTopologyForDescriptor:descriptor];
        DLog(@"[TideyRuntimeRehydration] phase=create_topology "
             "revision=%lld session=%@ succeeded=%@",
             descriptor.revision,
             descriptor.target.tmuxSession ?: @"",
             succeeded ? @"YES" : @"NO");
        TideyRuntimeLogOutcome(
...
- (void)resumeAgentInPanel:(NSString *)panelID
    ...
    dispatch_async(_queue, ^{
        ...
        DLog(@"[TideyRuntimeRehydration] phase=resume_agent panel=%@ "
             "revision=%lld session=%@ succeeded=%@",
             panelID,
             descriptor.revision,
             descriptor.target.tmuxSession ?: @"",
...
- (void)resumeDirectAgentInPanel:(NSString *)panelID
    ...
    [session startProgram:command
     ...
     webViewConfiguration:nil
        ...
        DLog(@"[TideyRuntimeRehydration] phase=resume_direct_agent "
             "panel=%@ revision=%lld succeeded=%@",
             panelID,
             descriptor.revision,
             succeeded ? @"YES" : @"NO");
        TideyRuntimeLogOutcome(
...
- (void)attachPanel:(NSString *)panelID
    ...
    [session startProgram:command
     ...
     webViewConfiguration:nil
               completion:^(BOOL succeeded) {
        DLog(@"[TideyRuntimeRehydration] phase=attach_panel panel=%@ "
             "revision=%lld session=%@ succeeded=%@",
             panelID,
             descriptor.revision,
             descriptor.target.tmuxSession ?: @"",
...
- (void)markPanelUnavailable:(NSString *)panelID
    ...
    PTYSession *session =
        [_windowController tideySelectedSessionForPanelIdentifier:panelID];
    DLog(@"[TideyRuntimeRehydration] phase=mark_unavailable panel=%@ "
         "revision=%lld session=%@",
         panelID,
         descriptor.revision,
         descriptor.target.tmuxSession ?: @"");
    TideyRuntimeLogOutcome(
...
@property(nonatomic, copy) NSString *tideyLastBroadcastWorkspaceSelectionIdentifier;
@property(nonatomic, copy) NSString *tideyLastBroadcastPanelSelectionIdentifier;
...
- (NSArray<NSDictionary *> *)tideySocketLogicalPanelSummariesForPanel:(PTYTab *)panel
                                                              workspace:(Workspace *)workspace
                                                         workspaceIndex:(NSInteger)workspaceIndex
                                                     startingPanelIndex:(NSInteger)startingPanelIndex;
...
@implementation PseudoTerminal {
    ...
    NSInteger _pendingWorkspaceIndexForInsertedPanel;
...
+ (NSArray<NSDictionary *> *)tideyNativeSessionPanelSummariesForCarrierPanelIdentifier:(NSString *)carrierPanelIdentifier
    ...
    for (NSDictionary *baseSummary in basePanelSummaries) {
        NSString *nativeSessionIdentifier = baseSummary[@"native_session_id"];
        NSString *panelIdentifier = [self
            tideyNativeSessionPanelIdentifierForCarrierPanelIdentifier:carrierPanelIdentifier
                                               nativeSessionIdentifier:nativeSessionIdentifier];
        if (panelIdentifier.length == 0) {
            continue;
        }
        NSMutableDictionary *summary = [NSMutableDictionary dictionaryWithDictionary:baseSummary];
        summary[@"panel_id"] = panelIdentifier;
        summary[@"carrier_panel_id"] = carrierPanelIdentifier;
        summary[@"native_session_id"] = nativeSessionIdentifier;
        summary[@"logical_kind"] = @"native_session";
...
- (NSDictionary *)tideyWorkspaceEventForKind:(NSString *)kind
    ...
    if (panelID.length > 0) {
        event[@"panel_id"] = panelID;
    }
    if (workspaceSummary.count > 0) {
        event[@"workspace"] = workspaceSummary;
    }
    if (panelSummary.count > 0) {
        event[@"panel"] = panelSummary;
    }
    return event;
}
...
- (void)tideyPostWorkspaceEventForKind:(NSString *)kind
                           workspaceID:(NSString *)workspaceID
                             ...
                             workspace:(NSDictionary *)workspaceSummary
                                 panel:(NSDictionary *)panelSummary {
    NSDictionary *event = [self tideyWorkspaceEventForKind:kind
                                               workspaceID:workspaceID
                                                   panelID:panelID
                                                 workspace:workspaceSummary
                                                     panel:panelSummary];
...
- (NSDictionary *)tideySocketWorkspaceSummaryForWorkspace:(Workspace *)workspace index:(NSInteger)index {
    ...
    NSMutableDictionary *summary = [NSMutableDictionary dictionaryWithDictionary:@{
        ...
        @"window_guid": self.terminalGuid ?: @"",
        @"panel_count": @([self tideySocketLogicalPanelCountForWorkspace:workspace]),
        @"selected_panel_id": [self tideySocketSelectedLogicalPanelIdentifierForWorkspace:workspace],
    }];
...
- (NSInteger)tideySocketLogicalPanelCountForWorkspace:(Workspace *)workspace {
    NSInteger count = 0;
    for (PTYTab *panel in workspace.panels) {
        count += panel.isTmuxTab ? 1 : panel.orderedSessions.count;
    }
    return count;
}
...
- (NSString *)tideySocketSelectedLogicalPanelIdentifierForWorkspace:(Workspace *)workspace {
    ...
    if (!panel || panel.isTmuxTab) {
        return carrierPanelIdentifier ?: @"";
    }
    return [[self class]
        tideyNativeSessionPanelIdentifierForCarrierPanelIdentifier:carrierPanelIdentifier
                                           nativeSessionIdentifier:panel.activeSession.guid] ?: @"";
}
...
- (NSArray<NSDictionary *> *)tideySocketLogicalPanelSummariesForPanel:(PTYTab *)panel
    ...
    if (panel.isTmuxTab) {
        NSMutableDictionary *summary = [NSMutableDictionary dictionaryWithDictionary:
            [self tideySocketPanelSummaryForPanel:panel
                                         workspace:workspace
                                    workspaceIndex:workspaceIndex
                                        panelIndex:startingPanelIndex]];
        summary[@"logical_kind"] = @"ordinary_tmux_window";
        return @[ summary ];
    }

    NSString *carrierPanelIdentifier = [self tideyPanelIdentifierForPanel:panel] ?: @"";
    NSMutableArray<NSDictionary *> *baseSummaries = [NSMutableArray array];
    NSInteger panelIndex = startingPanelIndex;
    for (PTYSession *session in panel.orderedSessions) {
        NSMutableDictionary *summary = [NSMutableDictionary dictionaryWithDictionary:
            [self tideySocketPanelSummaryForSession:session
                                              panel:panel
                                          workspace:workspace
                                     workspaceIndex:workspaceIndex
                                         panelIndex:panelIndex
                                    panelIdentifier:carrierPanelIdentifier]];
        summary[@"native_session_id"] = session.guid ?: @"";
        summary[@"selected"] = @([workspace selectedPanel] == panel && panel.activeSession == session);
        [baseSummaries addObject:summary];
        panelIndex++;
    }
    return [[self class]
        tideyNativeSessionPanelSummariesForCarrierPanelIdentifier:carrierPanelIdentifier
                                               basePanelSummaries:baseSummaries];
}
...
- (NSDictionary *)tideySocketPanelSummaryForPanelIdentifier:(NSString *)panelIdentifier {
    ...
    if (nativeIdentity) {
        PTYSession *session = [self tideySelectedSessionForPanelIdentifier:panelIdentifier];
        NSInteger logicalPanelIndex =
            [self tideySocketLogicalPanelIndexForPanel:panel session:session workspace:workspace];
        if (!session || logicalPanelIndex == NSNotFound) {
            return nil;
        }
        NSMutableDictionary *baseSummary = [NSMutableDictionary dictionaryWithDictionary:
            [self tideySocketPanelSummaryForSession:session
                                              panel:panel
                                          workspace:workspace
                                     workspaceIndex:workspaceIndex
                                         panelIndex:logicalPanelIndex
                                    panelIdentifier:panelIdentifier]];
        baseSummary[@"native_session_id"] = session.guid;
        baseSummary[@"selected"] = @([workspace selectedPanel] == panel && panel.activeSession == session);
        return [[[self class]
            tideyNativeSessionPanelSummariesForCarrierPanelIdentifier:nativeIdentity[@"carrier_panel_id"]
                                                   basePanelSummaries:@[ baseSummary ]] firstObject];
    }
    if (panel.isTmuxTab) {
        NSInteger logicalPanelIndex =
            [self tideySocketLogicalPanelIndexForPanel:panel session:panel.activeSession workspace:workspace];
        NSMutableDictionary *summary = [NSMutableDictionary dictionaryWithDictionary:
            [self tideySocketPanelSummaryForPanel:panel
                                         workspace:workspace
                                    workspaceIndex:workspaceIndex
...
- (NSDictionary *)tideySocketPanelListForWorkspaceIdentifier:(NSString *)workspaceIdentifier {
    ...
    NSInteger logicalPanelIndex = 0;
    for (PTYTab *panel in workspace.panels) {
        NSArray<NSDictionary *> *logicalPanels =
            [self tideySocketLogicalPanelSummariesForPanel:panel
                                                  workspace:workspace
                                             workspaceIndex:workspaceIndex
                                         startingPanelIndex:logicalPanelIndex];
...
- (NSDictionary *)tideySocketCreationResultForSession:(PTYSession *)session {
    ...
    NSString *panelIdentifier = panel.isTmuxTab
        ? [self tideyPanelIdentifierForPanel:panel]
        : [[self class]
            tideyNativeSessionPanelIdentifierForCarrierPanelIdentifier:[self tideyPanelIdentifierForPanel:panel]
                                               nativeSessionIdentifier:session.guid];
    NSDictionary *panelSummary = [self tideySocketPanelSummaryForPanelIdentifier:panelIdentifier];
    return @{
        @"workspace": workspaceSummary ?: @{},
        @"panel": panelSummary ?: @{},
    };
...
- (BOOL)tideyRenamePanelWithIdentifier:(NSString *)panelIdentifier title:(NSString *)title {
    ...
    if (nativeIdentity) {
        ...
        [self setName:trimmed forSession:session];
    } else {
    ...
    [self tideyPostWorkspaceEventForKind:@"panel_updated"
                             workspaceID:[self tideyWorkspaceIdentifierForWorkspace:workspace]
                               ...
                               workspace:[self tideySocketWorkspaceSummaryForWorkspace:workspace index:workspaceIndex]
                                   panel:[self tideySocketPanelSummaryForPanelIdentifier:panelIdentifier]];
    return YES;
}
...
- (NSInteger)indexOfWorkspaceContainingPanel:(PTYTab *)panel {
    ...
    for (NSInteger i = 0; i < self.workspaces.count; i++) {
        if ([[self workspaceAtIndex:i].panels containsObject:panel]) {
            return i;
        }
    }
    return NSNotFound;
...
- (void)updateSelectedPanelIndexFromVisibleTabSelection {
    ...
    NSInteger index = [workspace.panels indexOfObject:currentTab];
    if (index != NSNotFound) {
...
- (void)adjustWorkspaceIndexesAfterRemovingWorkspaceAtIndex:(NSInteger)removedIndex {
    if (_lastSelectedWorkspaceIndex == removedIndex) {
        _lastSelectedWorkspaceIndex = -1;
    } else if (_lastSelectedWorkspaceIndex > removedIndex) {
        _lastSelectedWorkspaceIndex--;
    }
    if (self.selectedWorkspaceIndex == removedIndex) {
        self.selectedWorkspaceIndex = -1;
    } else if (self.selectedWorkspaceIndex > removedIndex) {
        self.selectedWorkspaceIndex--;
    }
}
...
- (BOOL)selectWorkspaceAtIndex:(NSInteger)index recordHistory:(BOOL)recordHistory {
    ...
    if (self.selectedWorkspaceIndex == index) {
        PTYTab *panel = workspace.selectedPanel;
        NSInteger panelIndex = [self indexOfTab:panel];
        if (panelIndex != NSNotFound) {
            [self tideyMarkWorkspaceReadAtIndex:index];
...
- (void)createTideyWorkspacePossiblyTmux:(BOOL)possiblyTmux {
    ...
    _pendingInsertedPanelShouldAssignInitialTitle = YES;
    [[iTermController sharedInstance] newSession:nil possiblyTmux:possiblyTmux index:nil];
}
...
- (void)removeTab:(PTYTab *)aTab {
    ...
    [self ensureTideyWorkspacesInitialized];
    NSInteger tideyWorkspaceIndex = NSNotFound;
    ...
    if (self.isShowingTideySidebar) {
        ...
        if (tideyWorkspaceIndex != NSNotFound) {
            Workspace *workspace = [self workspaceAtIndex:tideyWorkspaceIndex];
            closedWorkspaceID = [self tideyWorkspaceIdentifierForWorkspace:workspace];
            ...
            if (removedPanelIndex != NSNotFound) {
                ...
                if (workspace.selectedPanelIndex >= workspace.panels.count) {
                    workspace.selectedPanelIndex = workspace.panels.count - 1;
                }
            }
            if (workspace.panels.count == 0) {
                if (self.workspaces.count > 1) {
                    tideyWorkspaceToReveal = MIN(tideyWorkspaceIndex, self.workspaces.count - 2);
                    [self.workspaces removeObjectAtIndex:tideyWorkspaceIndex];
                    [self adjustWorkspaceIndexesAfterRemovingWorkspaceAtIndex:tideyWorkspaceIndex];
                } else {
                    [self.workspaces removeAllObjects];
                    self.selectedWorkspaceIndex = -1;
                    _lastSelectedWorkspaceIndex = -1;
                    tideyShouldCloseWindow = YES;
                }
            } else if (tideyWorkspaceIndex == self.selectedWorkspaceIndex) {
                tideyWorkspaceToReveal = tideyWorkspaceIndex;
            }
        }
    }
    ...
    if (closingLastPanelInWorkspace && closedWorkspaceSummary) {
        [self tideyPostWorkspaceEventForKind:@"workspace_closed"
                                 workspaceID:closedWorkspaceID
    ...
    if ((self.isShowingTideySidebar ? tideyShouldCloseWindow : ([_contentView.tabView numberOfTabViewItems] <= 1)) &&
        ...
        if (aTabViewItem && [_contentView.tabView indexOfTabViewItem:aTabViewItem] != NSNotFound) {
            [_contentView.tabView removeTabViewItem:aTabViewItem];
        }
        if (self.isShowingTideySidebar && tideyWorkspaceToReveal != NSNotFound) {
            [self showWorkspaceAtIndex:tideyWorkspaceToReveal];
        } else if (_tideyPreservingWindowFrame || self.isShowingTideySidebar) {
...
- (IBAction)closeCurrentTab:(id)sender {
    if (self.isShowingTideySidebar) {
        ...
        Workspace *workspace = self.selectedWorkspace;
        if (workspace) {
            PTYTab *tab = self.currentTab ?: workspace.selectedPanel;
            if (tab) {
                [self performTideyWorkspaceMutationPreservingWindowFrame:^{
...
- (void)tideyRehydrateRuntimeDescriptors:
    ...
    for (NSString *panelID in panelIDs) {
        ...
        DLog(@"[TideyRuntimeRehydration] phase=admit panel=%@ "
             "descriptor_version=%ld revision=%lld session=%@ "
             "native_outcome=%ld",
             panelID,
...
- (void)tabView:(NSTabView *)tabView didSelectTabViewItem:(NSTabViewItem *)tabViewItem {
    ...
    if (self.isShowingTideySidebar && !tideySuppressWorkspaceTransitionSideEffects) {
        ...
        if (workspaceIndex != NSNotFound) {
            ...
            Workspace *workspace = [self workspaceAtIndex:workspaceIndex];
            NSInteger panelIndex = [workspace.panels indexOfObject:tab];
            if (panelIndex != NSNotFound) {
                workspace.selectedPanelIndex = panelIndex;
            }
    ...
    if (!tideySuppressWorkspaceTransitionSideEffects &&
        ...
        NSDictionary *workspaceSummary = [self tideySocketWorkspaceSummaryForWorkspaceIdentifier:currentWorkspaceID];
        NSDictionary *panelSummary = currentPanelID.length > 0 ? [self tideySocketPanelSummaryForPanelIdentifier:currentPanelID] : nil;
        [self tideyPostWorkspaceEventForKind:@"workspace_selected"
                                 workspaceID:currentWorkspaceID
                                     panelID:currentPanelID
                                   workspace:workspaceSummary
                                       panel:panelSummary];
        self.tideyLastBroadcastWorkspaceSelectionIdentifier = currentWorkspaceID;
    }
    if (!tideySuppressWorkspaceTransitionSideEffects &&
        currentPanelID.length > 0 &&
        ![currentPanelID isEqualToString:self.tideyLastBroadcastPanelSelectionIdentifier]) {
        NSDictionary *workspaceSummary = currentWorkspaceID.length > 0 ? [self tideySocketWorkspaceSummaryForWorkspaceIdentifier:currentWorkspaceID] : nil;
        NSDictionary *panelSummary = [self tideySocketPanelSummaryForPanelIdentifier:currentPanelID];
        [self tideyPostWorkspaceEventForKind:@"panel_selected"
                                 workspaceID:currentWorkspaceID
                                     panelID:currentPanelID
                                   workspace:workspaceSummary
                                       panel:panelSummary];
...
- (void)insertTab:(PTYTab*)aTab atIndex:(int)anIndex {
    ...
    if ([_contentView.tabView indexOfTabViewItemWithIdentifier:aTab] == NSNotFound) {
        ...
        if (!_tideySwitchingWorkspace && self.windowInitialized && !_fullScreen && !_restoringWindow) {
...
- (NSArray*)allSessions
{
    ...
    if (self.isShowingTideySidebar) {
        ...
        for (Workspace *workspace in self.workspaces) {
            for (PTYTab *panel in workspace.panels) {
                [result addObjectsFromArray:[panel sessions]];
            }
        }
        return result;
    }
