...
final class CodexAppServerConnection {
    ...
    func submitApproval(promptID: String,
        ...
        submitAdmissionHook?()
        return try completeSubmit(promptID: promptID,
    ...
    // Codex `item/tool/requestUserInput`: answers map keyed by question id.
    // Identical fail-closed lifecycle to option approvals — the flushed
    // response is NOT a resolution; only serverRequest/resolved (or a
    // turn/expiry terminal) terminalizes the request.
    @discardableResult
    func submitUserInput(promptID: String,
        ...
        submitAdmissionHook?()
        return try completeSubmit(promptID: promptID,
                                  outcome: approvalStore.beginSubmitUserInput(promptID: promptID,
                                                                              answers: answers,
                                                                              clientRequestID: clientRequestID,
                                                                              lifecycleToken: lifecycleToken))
    }

    private func completeSubmit(promptID: String,
                                outcome: CodexAppServerApprovalPromptStore.BeginSubmitOutcome) throws -> CodexAppServerApprovalSubmitOutcome {
        switch outcome {
