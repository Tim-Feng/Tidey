...
struct CodexAppServerApprovalRequest: Sendable {
    ...
    init?(method methodName: String,
        ...
        guard let method = CodexAppServerApprovalMethod(rawValue: methodName),
              ...
              let startedAtMs = params["startedAtMs"]?.intValue else {
            return nil
        }
    ...
    func makePrompt(epoch: String) -> InteractivePrompt {
        InteractivePrompt(promptID: promptID(epoch: epoch),
                          vendor: "codex",
                          source: promptSource,
                          title: title,
                          body: body,
    ...
    private var approvalOptions: [CodexAppServerApprovalOption] {
        ...
        switch method {
        case .permissions:
            preconditionFailure("permissions options are constructed above")
        case .commandExecution:
            return [
