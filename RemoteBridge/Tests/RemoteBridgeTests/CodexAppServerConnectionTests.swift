...
final class CodexAppServerConnectionTests: XCTestCase {
    ...
    // Round 7C P0-E: a superseded-payload-changed replacement must publish
    // the REPLACEMENT opener before the old-lifecycle terminal — never the
    // reverse — so the Hub's active-prompt map is never empty across the
    // transition. Verified two ways: (1) publish order via a shared
    // tracker, and (2) a deterministic interleaving test wired through a
    // REAL AgentEventHub — a matching continuation published exactly during
    // the old-terminal-still-pending window must never appear live/in
    // fetch, the new card must remain active, and the old terminal (its
    // OWN, different lifecycle token) must never close the new one.
    func testSupersededPayloadChangeReplacementOpenerPublishesBeforeOldTerminal() throws {
        ...
        let connection = Self.makeApprovalConnection(
            ...
            onInteractivePrompt: { envelope in
                promptEnvelopes.append(envelope)
                order.append("prompt:\(envelope.event.eventID)")
                hub.publish(envelope.event)
            },
            onInteractivePromptResolved: { event in
                order.append("resolved:\(event.metadata?["reason"] ?? "?"):\(event.metadata?["lifecycle_token"] ?? "?")")
                hub.publish(event)
            })

        // Establish a tracked turn so a continuation has something to defer
        // against.
        hub.publish(AgentEvent(eventID: "anchor-1", seq: 1, vendor: "codex", workspaceID: "workspace-1",
                               sessionID: "session-1", timestamp: "2026-01-01T00:00:00Z", type: .thinking,
                               role: nil, text: nil, name: nil, input: nil, output: nil, toolCallID: nil,
                               metadata: ["reason": "task_started", "turn_id": "turn-1"]))

        var interleavedContinuationWasStored = false
        hub.postStoreDeliveryHook = { [weak hub] event in
            // Fires the instant the REPLACEMENT opener (the SECOND
            // interactivePrompt stored for this session) lands — precisely
            // the window before the old terminal has published.
            guard event.type == .interactivePrompt,
                  promptEnvelopes.count == 2 else {
                return
            }
            hub?.postStoreDeliveryHook = nil
            let interleaved = AgentEvent(eventID: "interleaved-continuation", seq: 2, vendor: "codex",
                                        workspaceID: "workspace-1", sessionID: "session-1",
                                        timestamp: "2026-01-01T00:00:01Z", type: .thinking, role: nil,
                                        text: nil, name: nil, input: nil, output: nil, toolCallID: nil,
                                        metadata: ["reason": "tool_call", "is_continuation": "true", "turn_id": "turn-1"])
            hub?.publish(interleaved)
            interleavedContinuationWasStored = hub?.fetch(workspaceID: "workspace-1", sessionID: "session-1", limit: 100)
                .events.contains { $0.eventID == "interleaved-continuation" } ?? true
        }
        ...
        XCTAssertEqual(snapshot.count, 3, "prompt, prompt(replacement), resolved(old terminal) in that order, got \(snapshot)")
        XCTAssertTrue(snapshot[0].hasPrefix("prompt:"))
        XCTAssertTrue(snapshot[1].hasPrefix("prompt:"), "the replacement opener must publish SECOND, got \(snapshot)")
        XCTAssertTrue(snapshot[2].hasPrefix("resolved:superseded:"),
                      "the old-lifecycle superseded terminal must publish LAST (after the replacement opener), got \(snapshot)")

        XCTAssertFalse(interleavedContinuationWasStored,
                       "a continuation interleaved during the replacement window must never store — the active-prompt map must never be empty across the transition")

        let events = hub.fetch(workspaceID: "workspace-1", sessionID: "session-1", limit: 100).events
        XCTAssertFalse(events.contains { $0.eventID == "interleaved-continuation" },
                       "the interleaved continuation must never appear in fetch either")
        let replacementPromptID = promptEnvelopes[1].prompt.promptID
        XCTAssertNotNil(hub.activeInteractivePrompt(workspaceID: "workspace-1", sessionID: "session-1", promptID: replacementPromptID),
                        "the new (replacement) card must remain active after the old terminal publishes — the old terminal's own (different) lifecycle token must never close it")
    }
