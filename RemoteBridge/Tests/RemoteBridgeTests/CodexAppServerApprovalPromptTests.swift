...
final class CodexAppServerApprovalPromptTests: XCTestCase {
    func testMapsCommandApprovalRequestToInteractivePrompt() throws {
        ...
        XCTAssertEqual(prompt.options.map(\.label), ["Approve", "Approve for session", "Decline", "Cancel"])
        XCTAssertEqual(prompt.options.map(\.inputSequence), ["accept", "acceptForSession", "decline", "cancel"])
        XCTAssertEqual(prompt.selectedIndex, 0)

        let response = try request.response(targetIndex: 1)
        XCTAssertEqual(response.objectValue?["decision"]?.stringValue, "acceptForSession")
    }
    ...
    func testPromptStoreResolvesPromptOnce() throws {
        ...
        let response = try store.resolve(promptID: prompt.promptID, targetIndex: 3)
        XCTAssertEqual(response.objectValue?["decision"]?.stringValue, "cancel")
    ...
    func testRejectsUnknownPromptOptionIndex() throws {
        ...
        XCTAssertThrowsError(try request.response(targetIndex: 4))
    }
}
