...
final class BridgePendingApprovalFetchMergeTests: XCTestCase {
    ...
    func testSubscribeRacePromptDeliveredOnceWithSnapshotMetadata() throws {
        ...
        delivered.append(contentsOf: gate.open(suppressing: Set(injected.map { BridgeAgentEventSuppressionKey(event: $0.event) })))
    ...
    func testReplaySuppressionCoversLiveEnvelopeArrivingAfterGateOpen() throws {
        ...
        delivered.append(contentsOf: injected)
        deliveredLock.unlock()
        let flushed = gate.open(suppressing: Set(injected.map { BridgeAgentEventSuppressionKey(event: $0.event) }))
        deliveredLock.lock()
    ...
    func testUnsubscribedSinkIsNotCalledByQueuedDrains() throws {
        // Cancellation linearization: deliveries already queued for a
        // subscriber must not invoke its sink after unsubscribe returned; a
        // replacement subscriber receives only its own events.
        let hub = AgentEventHub()
        var oldSinkCalls = 0
        ...
        // Queued for the OLD subscriber while the drain is jammed.
        _ = publishAssistantEvent(hub: hub, text: "queued for old")
    ...
    func testGateOpenStillDeliversUnreplayedBufferedEvents() throws {
        ...
        hub.drainDeliveriesForTesting()
        delivered.append(contentsOf: gate.open(suppressing: [BridgeAgentEventSuppressionKey(sessionID: "session-1", eventID: "some-other-event", seq: 999)]))
        XCTAssertEqual(delivered.map(\.event.eventID), [live.eventID],
                       "suppression is by (session, eventID, seq); an unrelated key must not block unreplayed buffered events")
    }
